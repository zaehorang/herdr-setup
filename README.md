...
**레포를 옮기면 동기화가 꺼진다.** `.zshrc` 에는 이 레포의 절대경로가 박히므로
클론을 옮기거나 지우면 그 줄이 헛주소를 가리킨다. 그때는 새 셸마다 이렇게
알린다 — 조용히 넘어가면 스킬이 낡아가는 걸 눈치챌 방법이 없기 때문이다.

```
herdr-setup 이 /옛/경로 에 없다. herdr 스킬 동기화가 꺼져 있음 (옮긴 위치에서 herdr-setup.sh 를 다시 실행할 것).
```

옮긴 자리에서 `./herdr-setup.sh` 를 다시 돌리면 경로가 갱신된다. 이미 들어간
줄은 손으로 지우거나 새 경로로 고칠 것 — `[4]` 단계는 `sync-skill.sh` 라는
문자열이 이미 있으면 건드리지 않는다.
